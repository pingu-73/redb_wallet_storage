// This file is taken from bdk_wallet's test suit and modified: https://github.com/bitcoindevkit/bdk_wallet/blob/master/wallet/tests/wallet.rs
...
fn wallet_is_persisted() -> anyhow::Result<()> {
    ...
    run(
        "store.redb",
        |path| Ok(redb_wallet_storage::RedbStore::create(path)?),
        |path| Ok(redb_wallet_storage::RedbStore::open(path)?),
    )?;
    Ok(())
}
...
fn wallet_should_persist_anchors_and_recover() {
    ...
    let mut db = redb_wallet_storage::RedbStore::create(db_path).unwrap();
...
fn test_thread_safety() {
    ...
    thread_safe::<PersistedWallet<redb_wallet_storage::RedbStore>>();
}
